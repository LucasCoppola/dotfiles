...
BarWidget {
  ...
  property bool dateRevealLatched: false
  readonly property bool alwaysShowIndicators: setting("alwaysShow", false) === true
  readonly property bool dateRevealRequested: bar && bar.centerSectionRevealHeld === true && bar.centerHoverRevealSuppressed !== true
  readonly property bool revealInactiveIndicators: alwaysShowIndicators || dateRevealRequested || (dateRevealLatched && (indicatorAreaHovered || indicatorItemHovered))

  onDateRevealRequestedChanged: {
    if (dateRevealRequested) dateRevealLatched = true
    else if (!indicatorAreaHovered && !indicatorItemHovered) indicatorHideTimer.restart()
  }
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if (!root.dateRevealRequested && !root.indicatorAreaHovered && !root.indicatorItemHovered)
        root.dateRevealLatched = false
    }
  }
